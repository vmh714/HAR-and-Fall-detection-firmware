...
*   **Phase 3 (Data Collection)**: Xây dựng luồng thu thập dữ liệu IMU. Tích hợp MPU6050, lấy mẫu chính xác 100Hz dùng PCNT hardware ngắt tự động, lọc nhiễu bằng Kalman 1D (`lib_kalman`), và thiết lập cơ chế gom Batch để đẩy raw data lên MQTT (phục vụ thu thập Dataset train model).
